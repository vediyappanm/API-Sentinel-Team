  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <GlassCard variant="elevated" className="p-5">
        ...
        {isLoading && <SkeletonLoader variant="chart" />}

        {isError && <QueryError message="Failed to load the sequence graph" onRetry={() => refetch()} />}

        {isEmpty && (
          <EmptyState
            icon={<GitBranch size={40} strokeWidth={1.2} />}
            title="No call chains yet"
            description="Sequence flows are learned from captured traffic. Connect the mitmproxy sensor or upload a HAR file under Traffic Capture to populate this graph."
          />
        )}
