      ...
      <div className="bg-bg-surface border border-border-subtle rounded-lg overflow-hidden flex flex-col min-h-[500px]">
        <div className="p-2.5 border-b border-border-subtle flex items-center justify-between bg-bg-base">
          ...
          <div className="flex items-center gap-4 text-xs text-muted-foreground">
            <span>1 – {sensors.length} of {sensors.length}</span>
          </div>
        </div>

        {isLoading ? <TableSkeleton columns={8} rows={3} /> : (
            ...
            <table className="w-full text-left border-collapse table-fixed min-w-[1000px]">
              ...
              <tbody className="divide-y divide-border-subtle">
                ...
                {sensors.length === 0 && !isLoading && (
                  <tr><td colSpan={6} className="px-4 py-12 text-center text-xs text-muted-foreground">No sensors found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
